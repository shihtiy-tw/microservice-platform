...
case $ENVIRONMENT in
  dev)
    echo "Connecting to MySQL CLI in development environment as $USER..."
  ...
  test)
    echo "Connecting to MySQL CLI in test environment as $USER..."
    ...
    if [ -n "$DATABASE" ]; then
        docker-compose -f docker/compose/docker-compose.base.yml -f docker/compose/docker-compose.test.yml \
  ...
  prod)
    echo "Connecting to MySQL CLI in production environment as $USER..."
    ...
    if [ -z "$PASSWORD" ]; then
        read -sp "MySQL Password for $USER: " PASSWORD
        echo
    fi
